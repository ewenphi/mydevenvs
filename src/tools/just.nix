      ...
      ${
        if config.mydevenvs.tools.just.check.enable then
          ''
            ...
            nix-checks:
              if "nix --version"; then \
                NIXPKGS_ALLOW_UNFREE=1 nix flake check --no-pure-eval --accept-flake-config --all-systems --extra-experimental-features flakes --extra-experimental-features nix-command --impure;\
              else \
